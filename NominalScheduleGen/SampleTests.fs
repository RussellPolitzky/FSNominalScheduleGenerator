...
let ``should be able to get the last balance for amortizing loan``() =
    let finalNominalBalance = getFinalNominalBalance initialNominal r basis dates guessPayment
    finalNominalBalance |> should (equalWithin 1e-7) 307147.6192407597

[<Fact>] 
let ``should be able to find payment for amortizing loan``() =
    let requiredPayment = findPaymentForAmortizingLoan initialNominal 0.0 r basis dates  
...
let ``should be able to find payment for amortizing loan with residual``() =
    let requiredPayment = findPaymentForAmortizingLoan initialNominal residual r basis dates 
...
let ``should be able to get the last balance for amortizing loan - list``() =
    let schedule = getNominalSchedule initialNominal r basis dates guessPayment
...
let ``should be able to find payment for amortizing loan with residual - list``() =
    let schedule = getAmortizingLoanSchedule initialNominal residual r basis dates
    //schedule |> List.iter (fun item -> printfn "%A" item) // print the schedule to test output 
